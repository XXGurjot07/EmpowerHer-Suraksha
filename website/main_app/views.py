...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"New Account Created Successfully: {username}")
            login(request, user)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                messages.error(
                    request, f"Invalid username or password {username} ")
        else:
...
def emergency_contact(request):
    ...
    context = {'contacts': contacts,
               'total_contacts': total_contacts, 'user': user}
...
def update_contact(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            name = form.cleaned_data.get('name')
            mail = form.cleaned_data.get('email')
            message = "Hello, " + name + "\nYour contact information as emergency contact by " + \
                curr.username + " has been updated."
            send_mail(mail, message)
            messages.info(request, f"An email has been sent to your contact!!")
...
def scholarships(request):
    return render(request, 'main_app/scholarships.html', {'title': 'scholarships'})